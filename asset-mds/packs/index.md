## FRLegends Asset Database Packs List

All packs here are filtered by character (A-Z, 0-9, etc).

Download [Skeleton Key](https://github.com/ethanlabs101/frlegends-skeleton-key) to use these assets, and much more!

***Note: Packs include ALL cars/liveries. Use Skeleton Key to save
all assets to disk for later or inject directly to your account!***

Please Select A Character To Navigate:

| Character|Amount|
|----------|------|
| [B](https://github.com/ethanlabs101/FRLegends-Asset-Database/blob/main/asset-mds/packs/packs-filter/B.md) | 1 |
| [E](https://github.com/ethanlabs101/FRLegends-Asset-Database/blob/main/asset-mds/packs/packs-filter/index.md) | 1 | 
| [F]( - ) | 1 |
| [J]( - ) | 1 |
| [S]( - ) | 1 |

[→ Return to asset database](https://github.com/ethanlabs101/FRLegends-Asset-Database)
